...
inline double bilinear_interp(double x, double y, double v11, double v12, double v21, double v22) 
{
    return (v11 * (1 - y) + v12 * y) * (1 - x) + (v21 * (1 - y) + v22 * y) * x;
}

Mat ImgWarp_MLS::setAllAndGenerate(const Mat &oriImg, const vector<Point_<int> > &qsrc, const vector<Point_<int> > &qdst, const int outW, const int outH, const double transRatio) 
{
			...
			for (di = 0; di < h; di++)
			{
				for (dj = 0; dj < w; dj++) 
				{
					...
					if (oriImg.channels() == 1)
						newImg.at<uchar>(i + di, j + dj) = bilinear_interp(ny - nyi, nx - nxi, oriImg.at<uchar>(nyi, nxi), oriImg.at<uchar>(nyi, nxi1), oriImg.at<uchar>(nyi1, nxi), oriImg.at<uchar>(nyi1, nxi1));
					else
					{
						for (int ll = 0; ll < 3; ll++)
							newImg.at<cv::Vec3b>(i + di, j + dj)[ll] = bilinear_interp(ny - nyi, nx - nxi, oriImg.at<cv::Vec3b>(nyi, nxi)[ll], oriImg.at<cv::Vec3b>(nyi, nxi1)[ll], oriImg.at<cv::Vec3b>(nyi1, nxi)[ll], oriImg.at<cv::Vec3b>(nyi1, nxi1)[ll]);
					}
				}
			}
		}
